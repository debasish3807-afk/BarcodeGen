      ...
      <Container size="xl" className="relative">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          {TESTIMONIALS.map((testimonial, index) => (
            <motion.div key={testimonial.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.08, ease: [0.16, 1, 0.3, 1] }} className="relative p-6 rounded-2xl bg-white dark:bg-surface-900/60 border border-surface-200/60 dark:border-surface-700/40 hover:border-primary-200/60 dark:hover:border-primary-800/40 hover:shadow-lg hover:shadow-primary-500/[0.04] hover:-translate-y-1 transition-all duration-300">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-surface-100 dark:border-surface-800">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-surface-500 dark:text-surface-400 truncate">{testimonial.role} — {testimonial.company}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="mt-12 text-center">
          <div className="inline-flex items-center gap-4 px-6 py-3 rounded-full bg-white dark:bg-surface-800/60 border border-surface-200/60 dark:border-surface-700/40 shadow-sm">
            ...
            <span className="text-sm font-semibold text-surface-700 dark:text-surface-300">{t.testimonials.overallRating}</span>
          </div>
        </motion.div>
      </Container>
    </Section>
